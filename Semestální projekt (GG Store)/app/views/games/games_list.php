<?php
/** @var array $games */
// Pokud tady nahoře nemáš session_start(), ujisti se, že se spouští jinde v aplikaci
?>
<!DOCTYPE html>
        ...
        <div class="container mx-auto px-6 py-4 flex flex-col md:flex-row justify-between items-center relative">
            ...
            <nav class="mt-4 md:mt-0 w-full md:w-auto">
                <ul class="flex flex-wrap justify-center md:justify-end items-center gap-6">
                    <li><a href="<?= BASE_URL ?>/index.php" class="text-slate-300 hover:text-white hover:drop-shadow-[0_0_8px_#C44DFF] transition-all font-medium tracking-wide">Nabídka her</a></li>

                    <?php if (isset($_SESSION['user_id'])): ?>
                        <li><a href="<?= BASE_URL ?>/index.php?url=game/create" class="bg-gradient-to-r from-[#C44DFF] to-[#8E24DD] hover:from-[#d170ff] hover:to-[#9f2bf2] text-white px-5 py-2.5 rounded-lg transition-all shadow-[0_0_15px_rgba(196,77,255,0.4)] hover:shadow-[0_0_25px_rgba(196,77,255,0.6)] border border-[#d170ff]/50 text-sm font-bold uppercase tracking-wider">+ Přidat hru</a></li>

                        <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
                        <li><a href="<?= BASE_URL ?>/index.php?url=user/index" class="text-[#a8b1ff] hover:text-white hover:drop-shadow-[0_0_8px_#a8b1ff] transition-all font-medium text-sm tracking-wide">Správa uživatelů</a></li>
                        <?php endif; ?>

                        <li class="text-slate-400 text-sm hidden sm:block">Ahoj, <span class="text-[#e2b3ff] font-semibold tracking-wider"><?= htmlspecialchars($_SESSION['user_name'] ?? 'Hráči') ?></span></li>
                        
                        <li><a href="<?= BASE_URL ?>/index.php?url=auth/logout" class="text-rose-400/80 hover:text-rose-400 hover:drop-shadow-[0_0_8px_#fb7185] transition-all text-sm uppercase tracking-wider font-semibold">Odhlásit</a></li>

                    <?php else: ?>
                        <li><a href="<?= BASE_URL ?>/index.php?url=auth/login" class="text-slate-300 hover:text-white hover:drop-shadow-[0_0_8px_#C44DFF] transition-all font-medium tracking-wide">Přihlásit</a></li>
                        <li><a href="<?= BASE_URL ?>/index.php?url=auth/register" class="bg-[#1A122E] hover:bg-[#2A1D47] text-white px-5 py-2.5 rounded-lg transition-all border border-[#C44DFF]/30 hover:border-[#C44DFF] shadow-inner font-medium">Registrace</a></li>
                    <?php endif; ?>
                </ul>
            </nav>
        </div>
            ...
            <?php else: ?>
                <?php foreach ($games as $game): ?>
                    <div class="bg-[#1A122E] border border-[#2A1D47] rounded-2xl overflow-hidden shadow-lg transition-all hover:border-[#C44DFF]/50 hover:shadow-[0_0_30px_rgba(196,77,255,0.15)] group">
                        ...
                        <!-- OPRAVENÝ OBRÁZEK: Použito $game['images'] -->
                        <div class="aspect-video bg-[#0F0A1A] relative overflow-hidden flex items-center justify-center">
                            
                            <?php if (!empty($game['images'])): ?>
                                <img src="<?= BASE_URL ?>/uploads/<?= htmlspecialchars($game['images']) ?>" 
